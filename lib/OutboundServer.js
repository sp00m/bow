...
const authenticate = (socket, timeout) => async function (token) {
  try {
    ...
    assert.assigned(socket.userId, "user id retrieved from token");
...
function onConnection(socket) {
  const timeout = setTimeout(() => {
    debug("Connection timeout reached (%dms)", this.config.timeout);
  ...
  socket
    ...
    .on("authenticate", authenticate(socket, timeout).bind(this));
}
...
module.exports = class OutboundServer {
  ...
  async stop() {
    if (this.started) {
      ...
      throw new Error("Trying to stop a not started OutboundServer");
    }
  }
