...
async function addUser(id, criteria) {
  if (this.usersById.has(id)) {
    ...
    this.usersById.set(id, { criteria, sockets: [] });
    scatterUser.call(this, id);
  }
...
async function connectToPubSub() {
  if (this.pubSubBuilder.isOperational()) {
    this.pubSub = await this.pubSubBuilder.build(`BOW_USER_${this.index}`);
    this.pubSub.onMessage(({ id, criteria }) =>
      addUser.call(this, id, criteria));
    this.addUser = (id, criteria) => new Promise((resolver) => {
...
class Middleware {
  ...
  constructor(index, getUserCriteriaById, pubSubBuilder) {
    this.index = index;
  ...
  async register(socket) {
    ...
    await this.addUser(socket.userId, userCriteria);
    if (this.usersById.has(socket.userId)) {
      this.usersById.get(socket.userId).sockets.push(socket);
    }
  }
  ...
  constructor(middlewares, pubSubBuilder) {
    ...
    middlewares.forEach((middleware, index) =>
      this.middlewaresByVersion.set(middleware.version,
        new Middleware(index, middleware.getUserCriteriaById, pubSubBuilder)));
    Object.seal(this);
  }
