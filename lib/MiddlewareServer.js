...
class Middleware {
  ...
  constructor(middleware, pubSubBuilder) {
    ...
    this.getCriteriaFromListenerDetails = middleware.getCriteriaFromListenerDetails;
  ...
  async register(socket) {
    const criteria = await this.getCriteriaFromListenerDetails(socket.listenerDetails);
    assert.criteria(criteria);
    cleanCriteria(criteria);
    if (socket.connected) {
      const listenerId = socket.listenerDetails.id;
      const listenerExists = refreshListener.call(this, listenerId, criteria);
      if (!listenerExists) {
        this.listenersById.set(listenerId, { criteria, sockets: new Set() });
        scatterListener.call(this, listenerId);
      }
      this.listenersById.get(listenerId).sockets.add(socket);
      this.shareListenerCriteria(listenerId, criteria);
    }
  }
