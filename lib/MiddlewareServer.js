...
function scatterListener(listenerId) {
  const listenerCriteria = this.listenersById.get(listenerId).criteria;
...
class Middleware {
  ...
  async register(socket) {
    const listenerCriteria = await this.getCriteriaFromListenerId(socket.listenerId);
    assert.criteria(listenerCriteria);
    if (socket.connected) {
      const listenerExists = refreshListener.call(this, socket.listenerId, listenerCriteria);
      if (!listenerExists) {
        this.listenersById.set(socket.listenerId, { criteria: listenerCriteria, sockets: new Set() });
