...
const debug = require("debug")("bow:MiddlewareServer");
  ...
  async start() {
    ...
    if (check.positive(this.config.logInterval)) {
      this.logInterval = setInterval(() => {
        debug("Middleware v%s (instance identified by %s) currently manages %s listener(s)", this.version, this.uuid, this.listenersById.size);
