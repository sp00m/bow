...
class Middleware {
  ...
  constructor(getUserById, predicates) {
  ...
  async register(socket) {
    ...
    if (check.not.assigned(existingUser)) {
      const userData = await this.getUserById(socket.userId);
      assert.assigned(userData, "user retrieved from id");
...
module.exports = class MiddlewareServer {
  ...
  async start() {
    this.middlewares.forEach((middleware) =>
      this.middlewaresByVersion.set(middleware.version, new Middleware(middleware.getUserById, middleware.predicates)));
    this.started = true;
  }
  ...
  async stop() {
    if (this.started) {
      ...
      throw new Error("Trying to stop a not started MiddlewareServer");
    }
  }
