...
const cleanCriteria = (criteria) => {
  Object.keys(criteria).forEach((criterionKey) => {
    ...
    if (check.array(criterionValue)) {
      criteria[criterionKey] = [...new Set(criterionValue)];
    }
  });
};
...
class Middleware {
  ...
  constructor(middleware, pubSubBuilder) {
    ...
    this.getCriteriaFromListenerId = middleware.getCriteriaFromListenerId;
  ...
  async register(socket) {
    const criteria = await this.getCriteriaFromListenerId(socket.listenerId);
    assert.criteria(criteria);
    cleanCriteria(criteria);
    if (socket.connected) {
      const listenerExists = refreshListener.call(this, socket.listenerId, criteria);
      if (!listenerExists) {
        this.listenersById.set(socket.listenerId, { criteria, sockets: new Set() });
        scatterListener.call(this, socket.listenerId);
      }
      this.listenersById.get(socket.listenerId).sockets.add(socket);
      this.shareListenerCriteria(socket.listenerId, criteria);
    }
  }

  remove(socket) {
    if (this.listenersById.has(socket.listenerId)) {
      const listener = this.listenersById.get(socket.listenerId);
      listener.sockets.delete(socket);
      if (0 === listener.sockets.size) {
        clearScatteredListener.call(this, socket.listenerId);
        this.listenersById.delete(socket.listenerId);
      }
    }
  }
