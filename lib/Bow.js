...
module.exports = class Bow {
  ...
  middleware(version, getUserCriteriaById) {
    const cleanedVersion = normalizeVersion(version);
    assert.nonEmptyString(cleanedVersion, "middleware's version");
    assert.function(getUserCriteriaById, "middleware's getUserCriteriaById");
    this.middlewares.push(deepFreeze(clone({ version: cleanedVersion, getUserCriteriaById })));
    return this;
  }
