...
module.exports = class Bow {
  ...
  middleware(config) {
    ...
    assert.function(config.getCriteriaByListenerId, "middleware's getCriteriaByListenerId");
    this.middlewares.push(deepFreeze(clone({
      version: normalizedVersion,
      getCriteriaByListenerId: config.getCriteriaByListenerId
    })));
    return this;
  }
  ...
  outbound(config) {
    ...
    assert.function(config.getListenerIdByToken, "outbound's getListenerIdByToken");
    assert.nonEmptyString(normalizedMiddlewareVersion, "outbound's middlewareVersion");
    this.outbounds.push(deepFreeze(clone({
      version: normalizedVersion,
      getListenerIdByToken: config.getListenerIdByToken,
      middlewareVersion: normalizedMiddlewareVersion
    })));
    return this;
  }
