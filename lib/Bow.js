...
module.exports = class Bow {
  ...
  middleware(version, getCriteriaByListenerId) {
    const normalizedVersion = normalizeVersion(version);
    assert.nonEmptyString(normalizedVersion, "middleware's version");
    assert.function(getCriteriaByListenerId, "middleware's getCriteriaByListenerId");
    this.middlewares.push(deepFreeze(clone({ version: normalizedVersion, getCriteriaByListenerId })));
    return this;
  }

  inbound(path, getMessageFromRequestBody, middlewareVersion) {
    const normalizedMiddlewareVersion = normalizeVersion(middlewareVersion);
    assert.nonEmptyString(path, "inbound's path");
    if (InboundServer.healthCheckPath === path) {
      throw new Error(`'${path}' is reserved, it cannot be used for an inbound`);
    }
    assert.function(getMessageFromRequestBody, "inbound's getMessageFromRequestBody");
    assert.nonEmptyString(normalizedMiddlewareVersion, "inbound's middlewareVersion");
    this.inbounds.push(deepFreeze(clone({ path, getMessageFromRequestBody, middlewareVersion: normalizedMiddlewareVersion })));
    return this;
  }

  outbound(version, getListenerIdByToken, middlewareVersion) {
    const normalizedVersion = normalizeVersion(version);
    const normalizedMiddlewareVersion = normalizeVersion(middlewareVersion);
    assert.nonEmptyString(normalizedVersion, "outbound's version");
    assert.function(getListenerIdByToken, "outbound's getListenerIdByToken");
    assert.nonEmptyString(normalizedMiddlewareVersion, "outbound's middlewareVersion");
    this.outbounds.push(deepFreeze(clone({ version: normalizedVersion, getListenerIdByToken, middlewareVersion: normalizedMiddlewareVersion })));
    return this;
  }
