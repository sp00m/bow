...
module.exports = class Bow {
  ...
  middleware(config) {
    ...
    assert.function(config.getCriteriaFromListenerDetails, "middleware's getCriteriaFromListenerDetails");
    this.middlewares.push(deepFreeze(clone({
      version: normalizedVersion,
      getCriteriaFromListenerDetails: config.getCriteriaFromListenerDetails
    })));
    return this;
  }
  ...
  outbound(config) {
    ...
    assert.function(config.getListenerDetailsFromToken, "outbound's getListenerDetailsFromToken");
    assert.nonEmptyString(normalizedMiddlewareVersion, "outbound's middlewareVersion");
    this.outbounds.push(deepFreeze(clone({
      version: normalizedVersion,
      getListenerDetailsFromToken: config.getListenerDetailsFromToken,
      middlewareVersion: normalizedMiddlewareVersion
    })));
    return this;
  }
